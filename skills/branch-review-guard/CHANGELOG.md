...
v0.8.1 主题：**评审范围收敛 + 规则体系纠错**。起因是两次真实提测评审各跑 1 小时+（9 文件小 diff），复盘定位出"编排层只有扩张性指令、没有收敛指令"的结构性缺陷，外加 v0.8.0 发版遗留的三处规则事故。改后同规模 diff 实测墙钟 15-20 分钟（降约 70%），P1 质量未降（孪生锁 P1 三维度交叉确认、还在新修复代码里抓出新问题）。

### Fixed（规则体系，v0.8.0 遗留）

- **8 条晋升规则漏开开关**：v0.8.0 CHANGELOG 写"首批 19 条晋升"，实际 8 条 `enabled: false`（晋升 commit 只搬文件没开开关），真正生效仅 11 条——代价是一次评审中至少 3 条 P1 基于失效规则产出、怀疑者花整轮善后。已全部置 true。
- **4 条规则只在插件缓存、未入库**：`calibration-repo-ddl-not-baseline` / `calibration-dead-defense-null-branch` / `calibration-dependency-attribution-by-review-mode` / `finding-new-test-missing-tag` 此前只写进机器级缓存（重装即失、无版本控制、别人机器上没有），已回收入库。
- **缓存 `config.yaml` 残留 git 冲突标记**：YAML 语法非法，仅因 agent 用眼读而未炸。已清并同步。
- **api-change-guard skill 注册静默失败（0.5.1 起即存在）**：`SKILL.md` frontmatter 的 description 未加引号且含"（dimension: api）"——裸值中的"冒号+空格"使 YAML 解析成嵌套映射、整个 frontmatter 解析失败，插件加载器静默跳过注册，编排器第 5 章一直在降级自评。改"dimension=api"消掉冒号；全仓扫描确认无同类残留。api-change-guard 升 0.2.7。

### Added / Changed（编排器核心，主 SKILL.md + orchestrate + commands）

- **范围边界总纲**：新增 `## 评审范围` 总纲 + `## 范围边界`（三条硬边界：变更文件才深读 / 存量只报顺路撞见的、不派轮次去挖、不进对抗验证队列 / 孪生检查只做直接触碰对象），并给"强制全覆盖"戴范围笼头（标题与绝对措辞绑定"变更文件"）。措辞刚性与"强制全覆盖"对等——上一版劝导性表述被强势旧指令压制、实测无效，是本版重写的直接原因。
- **规模分档（分档表为唯一判据定义）**：轻量（<5 文件）/ 标准（5-15）/ 重型（>15；或 10-15 且 ≥2 高风险面）。**文件数是主导维度，高风险面数量不单独触发升档**（高风险面多的小 diff 应"加深"不"加轮"）；标准/轻量档一律不开 loop-until-dry 多轮、至多 1 轮定向聚焦。开局判档写进报告第 1 章。
- **loop-until-dry 收敛判据绑定范围**：只有落在变更文件内的新发现才重置收敛计数器——否则搜索范围扩到全仓后"连续 2 轮零新增"永不达成，必跑满 8 轮安全阀（实测跑到第 5 轮的根因）。
- **DLP 对策改"开局实测"**：删除所有写死的通道可信度断言（两份报告曾给出互相矛盾的结论），改为开局 30 秒对拍自检（`wc -l <file>` vs `wc -l < file` 判 mmap 失明等 4 探针），按实测结果选取证通道并写进报告第 1 章；补归因纪律（读法失明 ≠ 文件加密）与写通道纪律（重定向写出的文件被 DLP 加密，用 Write/cp）。实战首秀：安全维度读到幻觉内容自行中止、锁定干净通道重派，未产生误报。
- **实测取证要求**（`## 可靠性规则`）：量化/行为断言必须附命令与原始输出，否则按"未核实"；否定型实测须换第二条通道复核。起因：某报告断言"全仓 `@Tag("test")` 零命中"，复核实测 45 处命中，险些据此改错规则。

### Added / Changed（产物路径对齐）

- **设计侧产物改落被设计项目 `docs/`**（与 PRD 同处、入库）：`:design` 完整报告与精炼稿直接落位，去掉"提示用户手动搬运"环节（没搬就静默无上下文）。design-panel 升 0.5.3。
- **评审侧产物统一 `<报告目录>`**：新增唯一定义节 + 产物对照表，消灭 rule-drafts 四种写法（其中两种指向不同目录）；LEDGER/FEEDBACK 改跟随 `<报告目录>`（此前写死项目根，安装器形态下与报告分家）。

### Added（工具）

- **`scripts/sync-plugin-cache.sh`**：仓库↔插件缓存双向同步 + 分叉检测（check/push/pull），查 rules 冲突标记、`enabled: false` 漏开、skills/commands/agents 内容分叉。定位：开发期快速验证的捷径 + 发版体检；规则事实来源是仓库，缓存只是分发副本。
...
- **P0/P1 交叉确认**（借鉴同事"双人 2/2"；`SKILL.md` 对抗节 + 工作流程第 8 步 + orchestrate 第 8 步 + `verify-findings.md` 新节 + report-template 第 1/10 章）：与怀疑者投票**正交**——投票查"发现对不对"（带着结论找反证），交叉确认查"有几个独立 reviewer 命中"（不带结论从零重看）。每条 P0 与 P1 另派一个独立 reviewer 重看：≥2 独立命中标 `✓✓交叉确认`、仅 1 命中标 `⚠单源`（建议人工复核）；为确认而派的 reviewer 常附带捞回新发现。P0/P1 都做（漏报/误判在 P1 同样常见，如同事 R2–R6 均为 P1），P2/Nit 不做。诚实边界：两 reviewer 同底模（本版起固定 opus），`✓✓` 是弱独立非真异构——强独立要靠换非 Claude 家族模型或人工复核。
- **子代理统一升 `model: opus`**（11 个 `agents/bru-*.md` + `agents/dsp-*.md` frontmatter `model: inherit`→`opus` + 新增 `SKILL.md ## 子代理模型档位` + CLAUDE.md 组件节同步）：确认 Claude Code 支持 per-agent 指定模型（四级优先级：`CLAUDE_CODE_SUBAGENT_MODEL` 环境变量 > 单次派发 model 参数 > frontmatter > 主会话），不计成本前提下每个评审/设计环节都用最强档提质量（承重的对抗验证/裁判/业务语义维度尤其吃推理）。**边界**：Claude Code model 取值只到 Claude 家族（opus/sonnet/haiku/fable），家族内是**弱异构**（同源、错误相关性高），故统一升 opus 而非混档换多样性——真异构（错误不相关）仍只能靠异构复审 SOP（主会话换非 Claude 家族 + 人工）。据此修正了 SKILL/verify-findings/SOP 里"同底模"的归因（原写 `model: inherit`，现为固定 opus）。可移植性：`model:` 是 Claude Code 专属字段，非 Claude 形态预期忽略、退回自身默认模型（【待运行时验证】，降级不报错）。
...
- **DLP 密文文件 fallback 链显式化**（真实执行暴露后修复；`SKILL.md ## DLP 透明加密环境对策` 第 1/3 条重构 + `completeness-critic.md` 加第 7 条密文覆盖核查）：实测发现原判断"Read 能绕过 DLP"**不准确**——Read 对大多数加密文件拿明文，但**部分文件即便 Read 也返回 `%TSD-Header` 密文**（本仓库是混合加密）。原机制靠"已提交文件走 git"兜住了这次（那些密文文件恰好都已提交），但暴露缺口：**未提交的新文件若 Read 也是密文，git 对象库救不了、会静默漏读**。纪律：单文件 Read 密文时按 fallback 链——已提交走 `git show`；未提交无兜底则**显式记入 `## 未覆盖风险`、禁止当已评审**；每个文件读取都要判一次密文（混合环境不能因首文件明文就假设全明文）。完整性核查加"密文覆盖核查"抓"对着密文假装读过"。
- **批次划分模块级完整性对账**（真实执行暴露后修复；`SKILL.md ## 大 Diff 分批` 第 1 步 + `completeness-critic.md` 第 1 条升级为模块级+文件级双重对账）：实测发现一次评审**整个外围模块（减重 controller）未被分进任何批次**而漏读——纯文件级覆盖率对账抓不到（模块没进批次回执，两边都缺、反显"一致"）。修法：估规模时先按顶层前缀归并出"全模块集"，分批必须覆盖每个模块（外围/看似无关的也不许整模块不列入）；完整性核查先做"变更模块集 vs 批次覆盖模块集"对账，漏掉整模块强制补审。
- **业务语义维度新增"模式六 值集一致性"**（真实执行暴露 R5 漏报后补；`review-business-invariant.md` 第 1 步加第 6 类对象 + 第 2 步加模式六 + `bru-business-invariant.md`/SKILL 维度映射/orchestrate/report-template §3.5 同步）：专抓"硬编码字符串列表 ↔ 来源枚举实际产出值"的错配——列表里有个永不匹配的值（拼错/大小写/旧名）或少了该有的值。命中 R5（踢线硬编码 `"Harmony"` 但 `AppClientType` 实际产出 `HMOS`→旧会话踢不掉、安全回归）这类"看着正常实则跨值漏"。靠 Read 枚举定义 + 逐值核对，拿不到定义标"待人工确认"。业务语义维度追问模式 5→6、对象类 5→6。
- **三条通用误报 calibration 规则**（实测证伪反哺；`rules/baseline/` 新增 3 条，默认开、栈无关）：把一次"报告转独立证伪"实测暴露的三类可复用误报固化为降噪校准——① `calibration-mechanism-mismatch-not-bypass`（指控"绕过 X 过滤/治理"前先 Read 确认 X 本就管辖该对象，X 从不治理它=机制错配非绕过，但剥离独立成立的真问题）；② `calibration-orthogonal-not-contradiction`（判"自相矛盾"前先确认两者真会同一响应/时刻共存，正交语义如"历史累计 vs 当前快照"从不共存=非矛盾）；③ `calibration-security-gate-not-degradable`（判"缺降级"前先分清装饰数据 vs 安全/授权闸门，闸门 fail-close 是预期非缺陷）。落 `baseline/` 而非 `discover-new/`，因这三类是任何 AI 评审都会犯的**跨项目普适**误报、非 SKG 栈特有。
- **异构复审 SOP 增"精度复核（独立证伪）"方向**（`docs/异构复审-SOP.md`）：原 SOP 只讲召回（换模型补漏报），补一个降误报方向——拿已产出的报告换纯证伪立场独立重跑揪误报，附标准证伪 prompt 骨架（单点证伪剧本）。实测"独立会话 + 立场翻转"是有效的弱异构、能揪出同模型一次编排没降住的误报；结果走 distill 闭环沉淀成 calibration（即上面三条）。边界：只查"已标的对不对"不查"没标的有没有漏"。
