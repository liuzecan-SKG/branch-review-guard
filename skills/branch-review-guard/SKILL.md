...
1. **估规模 + 列全模块清单**：`git diff --stat <base>...HEAD` 拿到变更文件总数与完整列表；**并按顶层模块/目录前缀归并出"本次变更涉及的全部模块集"**（如 `xxx-modules/<模块名>/`）。分批方案必须覆盖这个模块集里的**每一个模块**——**外围/看似无关的模块（如减重、活动、后台修数接口）也必须分进某个批次，不许因"不属本功能主链路"就整个模块不列入**（历史教训：无鉴权 GET 全表重写这类问题正藏在外围模块）。完整性核查会做"模块集 vs 批次覆盖模块"对账（见 `## 对抗性验证与完整性核查`），漏掉整模块会被抓出来补审。
...
> 这是**横切纪律**，影响每一个维度、每一个子代理。某些主机装了 DLP（数据防泄漏）透明加密，工作区文件**内容**被加密（文件名不加密）。于是任何按文件**内容**搜索的工具——Bash 的 `grep`/`rg`/`sed`、Grep 工具、外部静态分析引擎——读到的都是密文，返回**"假零命中"**。它双向都有害：reviewer 找调用方会"搜不到"→漏报；怀疑者核实证据会"找不到反证"→把真发现误杀。Read 工具**对大多数加密文件能走解密层拿到明文**，但**实测并非万能——某些文件即便 Read 也返回密文**（内容以 `%TSD-Header` 开头）。本节把兜底变成机制，尤其钉死"Read 也读到密文"这个真实缺口。

1. **开局明文自检（工作流程第 1 步"确定范围"里顺手做）**：从变更清单取一个已知源码文件（`.java`/`.ts`/`.go` 等），用 **Read** 读出来，检查内容是否以 `%TSD-Header` 开头、或含大量 NUL 字节。
   - Read 拿到密文 → 本环境 **DLP 加密**（内容检索按下面纪律切换），在报告"评审范围与方法"声明「DLP 透明加密环境：内容检索已切 Read-only + git fallback」。
   - Read 拿到明文、但抽查另一个文件是密文 → **混合加密环境**（本仓库实测就是这种：部分文件 Read 明文、部分 Read 密文），同样按 DLP 纪律走，且**每个文件读取时都要判一次**（不能因为第一个文件明文就假设全部明文）。
   - 全部抽查都是明文 → 正常环境，Grep 可正常用于内容检索。
2. **内容检索去 grep 化（DLP 加密时）**：**文件名用 Glob、文件内容用 Read**；**禁止用 Bash 的 `grep`/`rg`/`sed` 或 Grep 工具按内容检索**。
3. **单文件 Read 读到密文时的 fallback 链（关键，防静默漏读）**：任何一个变更文件 Read 回来是 `%TSD-Header`/密文（不是明文），**不许当"读过"直接跳过**，按下面顺序兜底：
   - ① **已提交文件** → `git show HEAD:<path>` / `git cat-file --filters <base>:<path>` 读 git 对象库明文（git 对象库不受工作区加密影响，可靠）；未暂存改动用 `git diff`、已暂存用 `git diff --cached` 取该文件的明文 hunk。
   - ② **未提交的新文件（untracked / 无 git 对象）** → **git 通道救不了**（对象库里没有）。此时若 Read 也只有密文，该文件**无法评审**：必须在报告 `## 未覆盖风险` **显式列出**「<path> Read 返回密文且无 git 兜底，本次未覆盖」，**严禁计入"已深读"、严禁当已评审**。
   - ③ fallback 成功拿到明文的，正常评审，但发现条目的取证标 `[git]` 而非 `[Read]`。
4. **否定型结论必须标注取证方式**：所有否定型断言（"无其它调用点 / 不存在 / 全仓无 / 未发现 X"）必须附取证标记 `[Read]` / `[git]` / `[Grep·仅非DLP可信]`。DLP 下凡 `[Grep]` 取证的否定结论**一律不可信**，由完整性核查（`completeness-critic.md`）强制退回 Read/git 重核；拿不出可信复核的，改写为「未核实」而非「不存在」。
5. **子代理同样适用**：派发给 `bru-*` 子代理的任务书里注入本节纪律（含第 3 条 fallback 链）；怀疑者（`bru-skeptic`）的"证据核实"视角、批评家（`bru-critic`）的取证核查，都以本节为准——子代理白名单里的 Grep 工具在 DLP 下只用于"已知非加密"或路径/文件名类查找，不用于内容否定取证。**任一子代理遇到"Read 密文 + 无 git 兜底"的文件，必须回传该文件为"未覆盖（密文致盲）"，不得静默略过。**
