...
2. 把 `$ARGUMENTS` 解析为该 skill 的模式与选项（`branch` / `diff` / `recent <N>` / `module <名>`、`--base`、`--dimensions`、`--thorough` 高风险批次 loop-until-dry 多轮扫描：连续 2 轮零新发现才停、轮数按 diff 自然收敛、仅 8 轮安全阀防死循环）。
