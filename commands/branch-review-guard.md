...
1. 调用本插件的 **`branch-review-guard` skill**，严格按其 `SKILL.md` 工作流程执行：确定范围 → 建立上下文（读 `*_DESIGN.md` / `*_CONTRACT.md` / commit message）→ 自动化先行(L1) → 加载 `rules/` 规则包 → 估规模分批 → 分维度评审 → 复用 `api-change-guard` / `endpoint-perf-review` → 汇总去重 → 产出单份中文报告。
