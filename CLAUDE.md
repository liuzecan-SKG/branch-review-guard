...
- `agents/bru-*.md` + `agents/dsp-*.md` — 插件形态下的 11 个只读子代理：**评审侧 8 个** `bru-*`（6 个维度评审 correctness / design / security / tests / observability / business-invariant，按批并行派发 + 对抗验证怀疑者 `bru-skeptic`——每条 P0/P1 × 3 视角降误报 + 对"正例/零发现"× 第 4 视角假设证伪捞漏报 + 完整性批评家 `bru-critic`，定稿前 1 个）；**设计侧 3 个** `dsp-*`（`dsp-designer` 设计代理按视角并行 N 实例 + `dsp-skeptic` 每方案 1 个质证 + `dsp-judge` 裁判定稿前 1 个）。**所有子代理的只读靠 frontmatter `tools: Read, Grep, Glob, Bash` 白名单保证**（不是 prompt 里的一句话）；**frontmatter 另统一 `model: opus`**（每环节用最强档提质量，见 SKILL.md `## 子代理模型档位`——Claude Code per-agent 指定只到 Claude 家族，家族内是弱异构故统一升 opus 而非混档，真异构靠异构复审 SOP；非 Claude 形态预期忽略该字段、降级不报错）。不支持子代理的环境自动退化为顺序执行。
